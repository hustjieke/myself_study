...
void MemTable::Add(SequenceNumber s, ValueType type,
                   const Slice& key,
                   const Slice& value) {
  // Format of an entry is concatenation of: // 条目的格式是串联的
...
// memtable对key的查找和遍历封装成MemTableIterator.底层直接使用SkipList的类Iterator接口.
...
bool MemTable::Get(const LookupKey& key, std::string* value, Status* s) {
  ...
  Table::Iterator iter(&table_);  // Table是跳表的typedef
