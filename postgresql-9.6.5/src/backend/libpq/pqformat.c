...
{
	initStringInfo(buf);  // buf->len=0,buf->cursor=0,buf->maxlen=1024;
	/* Reserve four bytes for the bytea length word */ // buf->data[]的头四个
	appendStringInfoCharMacro(buf, '\0');
	appendStringInfoCharMacro(buf, '\0');
	appendStringInfoCharMacro(buf, '\0');
	appendStringInfoCharMacro(buf, '\0');  // 执行完之后,buf->len=4
}
...
pq_endtypsend(StringInfo buf)
{
	...
	Assert(buf->len >= VARHDRSZ);  // 因为buf肯定是预留了4个字节的,所以>=VARHDRSZ(int32)
