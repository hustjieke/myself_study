...
#define MUL_GUARD_DIGITS	2	/* these are measured in NBASE digits */// int16的个数,基数:NBASE
 ...
 * NUMERIC_NEG, it follows the NumericLong format.  If they are NUMERIC_NAN,  // 2个字节,高位为:0xC000,二进制:11000000
 ...
 * the future.
 *
 * In the NumericShort format, the remaining 14 bits of the header word  // 高两位用于标记是否NAN
 ...
 * stored separately in n_weight.  // 之前的版本只有NumericLong format,新版本相当于做了优化,每个number存储少了两个字节
 *
 * NOTE: by convention(惯例), values in the packed form(紧凑形式) have been stripped(剥掉) of
 * all leading and trailing zero digits (where a "digit" is of base NBASE).
 * In particular, if the value is zero, there will be no digits at all!
 * The weight is arbitrary(任意的) in that case, but we normally set it to zero.
 */
...
// 没有long么?(对于Long,没必要去检测每一位,为了更好的效率,见下面的注释)
#define NUMERIC_SIGN_MASK	0xC000  // 作用是和header进行与运算,取出高两位bit的值,方便与特征码进行比较
...
#define NUMERIC_HDRSZ	(VARHDRSZ + sizeof(uint16) + sizeof(int16))  // 向前兼容,针对NumericLong
#define NUMERIC_HDRSZ_SHORT (VARHDRSZ + sizeof(uint16))  // NumericShort
 ...
 * can just look at the high bit, for a slight efficiency gain.(为了获得轻微的效率提升)
 */
 ...
 * ndigits, and digits[] array.  // dscale只是限定显示范围?
 *
 ...
 *
 * dscale, or display scale, is the nominal(名义上的) precision expressed as number
 ...
 */
// int16的数组
// 权值是进制的(位数-1)的幂,比如9999 9999 9999.9999,三个int16,权值是2而不是3(原理跟10进制权值一样的算法)
// 相对于int16来说第一个数的权值,就是整数部分int16个数
// 目前看是计算时实际开辟的buf空间
// *digits是运算后不包括Leading/trailing 0的紧凑数组
...
/* ----------
 * Some preinitialized constants  // 预初始化常数
 * ----------
 */
...
#ifdef HAVE_INT128  // 待定分析,暂不会到int128
...
static void div_var_fast(NumericVar *var1, NumericVar *var2, NumericVar *result,
			 int rscale, bool round);
static int	select_div_scale(NumericVar *var1, NumericVar *var2);  // 返回合适的scale,什么作用?暂不列入分析
...
numeric_maximum_size(int32 typmod)
{
	...
	if (typmod < (int32) (VARHDRSZ))  // 如果typmod小于var header size,直接返回-1
		return -1;

	/* precision (ie, max # of digits) is in upper bits of typmod */
    // 取出高两位字节,这里的precision就是指的number的数字个数,不是int16个数
	precision = ((typmod - VARHDRSZ) >> 16) & 0xffff;  // 取出高两位字节
	 ...
	 * NumericDigit.
	 */
    // 2*(DEC_DIGITS - 1)的作用是考虑到最开始的两个的数字分别用一个int16存,
    // 这样就必须加上可能剩余的6位数才算是最大的要保存的位数,见注释说明
 ...
 * numeric_out_sci() -
 *
 *	Output function for numeric data type in scientific notation.  // 科学计算法
 */
char *
 ...
 */
Datum  // typedef uintptr_t Datum;
numeric_send(PG_FUNCTION_ARGS)  //入参宏展开:FunctionCallInfoData * fcinfo
{
    // 对于PG_GETARG_NUMERIC宏,在这里不用过多考虑细节,就是提取出numeric数据指针
	Numeric		num = PG_GETARG_NUMERIC(0);  // typedef struct NumericData *Numeric;
	NumericVar	x;  // 结构体
	StringInfoData buf;
	int			i;

	init_var_from_num(num, &x);  // 从num中取出对应信息用来初始化x,此时x->buf=NULL

	pq_begintypsend(&buf);  // 初始化buf并预留buf->data[1024]的头四个指针,buf->len=4
	...
	for (i = 0; i < x.ndigits; i++)  // 之所以循环拷贝,是pq_sendint这个函数的实现就是每次拷贝1/2/4个字节
 ...
 *	Note: its dscale is not touched(涉及).
 */
static void
...
set_var_from_str(const char *str, const char *cp, NumericVar *dest)
{
	...
	if (*cp == '.')
	{
		have_dp = TRUE;  // have decimal point
		cp++;
	}
	...
	/* leading padding for digit alignment(校准) later */
	memset(decdigits, 0, DEC_DIGITS);  // 另外一个DEC_DIGITS放在尾部做校准,见下文代码
	i = DEC_DIGITS;
	...
	while (*cp)
	{
		if (isdigit((unsigned char) *cp))
		{
			decdigits[i++] = *cp++ - '0';
			if (!have_dp)
				dweight++;  // 此处是10进制的权值,dweight从-1开始递增
			else
				dscale++;  // 小数位数++
		}
		else if (*cp == '.')
		...
		else  // 如果不是数字又不是小数点呢?其它字符不作处理么?
			break;
	}

	ddigits = i - DEC_DIGITS;  // 数字个数,到这里还不是int16的个数
	/* trailing padding for digit alignment later */
	memset(decdigits + i, 0, DEC_DIGITS - 1);  // 一头一尾各一个DEC_DIGITS用作校准
	...
	if (*cp == 'e' || *cp == 'E')
	{
		...
		cp++;
		exponent = strtol(cp, &endptr, 10);  // 字符转成long数值,记录最后一个数字的位置
		if (endptr == cp)  // e/E后面一个数字都没有
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
					 errmsg("invalid input syntax for type numeric: \"%s\"",
							str)));
		cp = endptr;  // cp 指向endptr位置
		 ...
		 */
        // <limits.h>中有INT_MAX和INT_MIN的宏定义可直接使用,max=2147483647,min=-2147483648
		if (exponent >= INT_MAX / 2 || exponent <= -(INT_MAX / 2))  // 为什么以INT_MAX作为比较没看懂
			ereport(ERROR,
					(errcode(ERRCODE_NUMERIC_VALUE_OUT_OF_RANGE),
					 errmsg("value overflows numeric format")));
		dweight += (int) exponent;  // "1.23E123"这种情形
		dscale -= (int) exponent;
		if (dscale < 0)  // 什么情况下<0?
			dscale = 0;
	}
	...
	/*
	 * Okay, convert pure-decimal(纯粹十进制的) representation to base NBASE.  First we need
	 * to determine the converted weight and ndigits.  offset is the number of
	 * decimal zeroes to insert before the first given digit to have a
	 * correctly aligned first NBASE digit.
	 */
	if (dweight >= 0)
		weight = (dweight + 1 + DEC_DIGITS - 1) / DEC_DIGITS - 1;  // 拆分的逻辑是?
	else
	...
	dest->sign = sign;
	dest->weight = weight;
	dest->dscale = dscale;  // 这个dscale到底是int16的个数还是小数的位数???
...
#error unsupported NBASE
	...
	pfree(decdigits);  // 释放辅助计算的内存

	/* Strip any leading/trailing zeroes, and normalize weight if zero */
	strip_var(dest);  // 标准化NumbericVar,不包含leading 0和trailing 0.
...
get_str_from_var(NumericVar *var)
{
	 ...
	 * as many as DEC_DIGITS-1 excess(超过,余量) digits at the end, and in addition we
	 * need room for sign, decimal point, null terminator. // 所以总共要多加(DEC_DIGITS-1+1+1+1)个
	 */
...
{
	int32		exponent;  // 指数
	NumericVar	denominator;  // 分母,共同特性
	NumericVar	significand;  // 尾数,有效位数
	int			denom_scale;  // 单位规模
	size_t		len;
	char	   *str;
	char	   *sig_out;
	 ...
	 */ // 意思是这个计算出的指数值对应小数点左边只有一位数字
	if (var->ndigits > 0)
	{
		exponent = (var->weight + 1) * DEC_DIGITS;  // 换算成10进制的权值

		/*
		 * Compensate for leading decimal zeroes in the first numeric digit by
		 * decrementing the exponent.  // 如果有leading有几个0,指数就递减几
		 */
		exponent -= DEC_DIGITS - (int) log10(var->digits[0]);  // 库函数
	}
	else
	{
		/*
	 ...
	 */
	if (exponent < 0)
		denom_scale = -exponent;  // 这是放大相应的10^指数么?
	else
		denom_scale = 0;  // 指数大于0就不用放大?
...
make_result(NumericVar *var)
{
	...
	if (sign == NUMERIC_NAN)
	{
		...
		SET_VARSIZE(result, NUMERIC_HDRSZ_SHORT);  // 注意宏用的指针类型强制转换
		result->choice.n_header = NUMERIC_NAN;
		/* the header word is all we need */

		dump_numeric("make_result()", result);  // 仅仅是输出内容,用于调试
		return result;
	}

	n = var->ndigits;  // 保存int16个数
	...
	while (n > 0 && digits[n - 1] == 0)  // 注意此时digits的位置可能已经移动,n的值也已经改变
		n--;
	...
	if (NUMERIC_WEIGHT(result) != weight ||  // 上面的weight--只是对第一个int16移除0,
 ...
 */  // 为什么要round??rscale:小数位数,非int16个数
static void
		...
		int rscale)
{
	 ...
	 */  // 考虑到效率的问题,尽量保持内循环次数更多,外循环次数更少
	if (var1->ndigits > var2->ndigits)
	{  // 交换var1和var2,保持外循环次数更少
		NumericVar *tmp = var1;
	...
	res_weight = var1->weight + var2->weight + 2;  // +2是最大的可能权值

	/*
	 * Determine the number of result digits to compute.  If the exact result
	 * would have more than rscale fractional digits, truncate the computation
	 * with MUL_GUARD_DIGITS(宏:2) guard digits, i.e., ignore input digits that
	 ...
	 */
	res_ndigits = var1ndigits + var2ndigits + 1;  // 最终结果int16个数
	maxdigits = res_weight + 1 + (rscale + DEC_DIGITS - 1) / DEC_DIGITS +
		MUL_GUARD_DIGITS;
	res_ndigits = Min(res_ndigits, maxdigits);

	if (res_ndigits < 3)  // 没看明白,MUL_GUARD_DIGITS的作用到底是什么?
	{
	 ...
	 * INT_MAX is noticeably(明显) larger than NBASE*NBASE, this gives us headroom(额外空间)
	...
	{
		int			var1digit = var1digits[i1];  // 从最后一个元素往前计算
 ...
 * div_var() -
 *
 *	Division on variable level. Quotient(商) of var1 / var2 is stored in result.
		...
		int rscale, bool round)
{
	...
	/* copy these values into local vars for speed in inner loop */
	int			var1ndigits = var1->ndigits;  // 不这么做的话空间开销太大
    ...
    // D1:规格化(即条件(Vn-1)>[b/2])中,对于d=[b/((Vn-1)+1)]=1,u<n+m>=0
	if (var2ndigits == 1)
	{  // n=1即只有一个元素时,采用简单算法,见算法D的描述部分,练习16
       // 就是最简单的多个数字除以一个数字,采用从前到后的除法
		/*
		...
		divisor1 = divisor[1];
		carry = 0;
        // 循环跟习题不一样,习题中循环的次数是被除数的总个数,这里直接是结果的数字开始
        // 当然,这里已经考虑到了被除数的第一位比除数小的情形了。
		for (i = 0; i < res_ndigits; i++)
		{
			carry = carry * NBASE + dividend[i + 1];
			res_digits[i] = carry / divisor1;  // 计算除数,第一次循环时可能为0
			carry = carry % divisor1;  // 求余,带到下一步运算
		}
	}
	else
	{
...
mod_var(NumericVar *var1, NumericVar *var2, NumericVar *result)
{
	 ...
	 * ----------
	 */
	div_var(var1, var2, &tmp, 0, false);  // 求商tmp

	mul_var(var2, &tmp, &tmp, var2->dscale);  // 求出 商*除数

	sub_var(var1, &tmp, result);  // 求模,注意和求余的差别
...
ceil_var(NumericVar *var, NumericVar *result)
{
    ...
    // 对于负数,截断就是ceil的结果,对于正数,如果不相等说明小数部分有值,则需要加1
...
sqrt_var(NumericVar *arg, NumericVar *result, int rscale)
{
	 ...
	 */
	alloc_var(result, 1);  // 假定值,用于迭代,无限逼近结果
	...
	for (;;)
	{
		...
		mul_var(result, &const_zero_point_five, result, local_rscale);  // 类似乘以0.5

		if (cmp_var(&last_val, result) == 0)  // 相等说明求得了结果
			break;
		set_var_from_var(result, &last_val);
	}
...
exp_var(NumericVar *arg, NumericVar *result, int rscale)
{
	...
	if (Abs(val) >= NUMERIC_MAX_RESULT_SCALE * 3)  // Abs(val) >= 6000,即e^6000以上就超出范围
		ereport(ERROR,
				(errcode(ERRCODE_NUMERIC_VALUE_OUT_OF_RANGE),
				 errmsg("value overflows numeric format")));

	/* decimal weight = log10(e^x) = x * log10(e) */
    // 以10进制为计算,10^dweight=e^x,自然权dweight值为如下:
	dweight = (int) (val * 0.434294481903252);  // The base 10 logarithm of e is: 0.434294481903252

	/*
	 * Reduce x to the range -0.01 <= x <= 0.01 (approximately) by dividing by
	 * 2^n, to improve the convergence rate of the Taylor series.  // 提高泰勒级数的收敛速度
	 */
	if (Abs(val) > 0.01)  // 自定义宏绝对值
	{
		NumericVar	tmp;
		...
		while (Abs(val) > 0.01)
		{
			ndiv2++;
			val /= 2;  // 自动转换double运算
			add_var(&tmp, &tmp, &tmp);  // tmp = tmp*2
		}

		local_rscale = x.dscale + ndiv2;
		div_var_fast(&x, &tmp, &x, local_rscale, true);  // 此时tmp=2^n,x=x/2^n
	 ...
	 * (dweight + rscale + 1)(应该写成(dweight+1+rscale) significant digits.  In addition, we have to
	 * raise the Taylor series result to the power 2^ndiv2, which introduces
	 * an error of up to around(大约) log10(2^ndiv2) digits, so work with this many
	 * extra digits of precision (plus a few more for good measure).  // 额外多计算一些精度数字,更好的测算
	 */  // lg2=0.301029995663981
	sig_digits = 1 + dweight + rscale + (int) (ndiv2 * 0.301029995663981);  // 后面是多出来的精度数
	sig_digits = Max(sig_digits, 0) + 8;  // 暂不明白

	local_rscale = sig_digits - 1;  // 暂不明白
	 ...
	 *
	 * Given the limited range of x, this should converge reasonably quickly.//这应该合理的收敛
	 * We run the series until the terms fall below the local_rscale limit.
	 */
	add_var(&const_one, &x, result);  // 首先是加1,result = 1 + x

	mul_var(&x, &x, &elem, local_rscale);  // x*x
	set_var_from_var(&const_two, &ni); // ni = 2
	div_var_fast(&elem, &ni, &elem, local_rscale, true);  // elem=x*x/2

	while (elem.ndigits != 0)
	{
		add_var(result, &elem, result);  // 从x*x/2开始累加

		mul_var(&elem, &x, &elem, local_rscale);  // x增加1
		add_var(&ni, &const_one, &ni);  // 除数阶数加1
	 ...
	 * as we proceed.
	 */
	while (ndiv2-- > 0)  // 之前x缩放了ndiv2倍x=x/ndiv2,现在还原,注意result*result=exp^x*ndiv2
	{
...
ln_var(NumericVar *arg, NumericVar *result, int rscale)
{
	 ...
	 */
	while (cmp_var(&x, &const_zero_point_nine) <= 0)  // 小于0.9,则放大
	{
		local_rscale = rscale - x.weight * DEC_DIGITS / 2 + 8;
		local_rscale = Max(local_rscale, NUMERIC_MIN_DISPLAY_SCALE);
		sqrt_var(&x, &x, local_rscale);
		mul_var(&fact, &const_two, &fact, 0);  // 保存好放大的倍数
	}
	while (cmp_var(&x, &const_one_point_one) >= 0)  // 大于1.1,则缩放
	{
		local_rscale = rscale - x.weight * DEC_DIGITS / 2 + 8;
		local_rscale = Max(local_rscale, NUMERIC_MIN_DISPLAY_SCALE);
		sqrt_var(&x, &x, local_rscale);
	 ...
	 *
	 * The convergence(收敛) of this is not as fast as one would like, but is
	 * tolerable(可容忍) given that z is small.
	 */
	local_rscale = rscale + 8;
...
power_var(NumericVar *base, NumericVar *exp, NumericVar *result)
{
	...
	if (exp->ndigits == 0 || exp->ndigits <= exp->weight + 1)  // 前面一个说明是0,后面一个条件说明是整数,没有小数
	{
		...
		{
			int			expval = (int) expval64;  // int64_t->int32_t,超过int32肯定就溢出了

			/* Test for overflow by reverse-conversion. */  // 逆转换
	...
	/* initial overflow test with fuzz(模糊) factor */
...
power_var_int(NumericVar *base, int exp, NumericVar *result, int rscale)
{
	...
	/* Handle some common special cases, as well as corner(角) cases */
	switch (exp)
	{
		case 0:
			 ...
			 const NumericDigit *var2digits, int var2ndigits, int var2weight)
{
	...
	{
		if (var1digits[i1++] != 0)  // 有权值,且Leading非0,那肯定var1>var2
			return 1;
		var1weight--;
	}
...
add_abs(NumericVar *var1, NumericVar *var2, NumericVar *result)
{
	...
	NumericDigit *res_digits;
	int			res_ndigits;  // 总共int16的个数
	int			res_weight;  // 整数部分10000的权值
	int			res_rscale,
				rscale1,  // 小数部分int16个数
				rscale2;
	int			res_dscale;  // 显示范围
	...
	res_weight = Max(var1->weight, var2->weight) + 1;  // +1是为了容纳超出MAX权值之后

	res_dscale = Max(var1->dscale, var2->dscale);  // 小数部分不用+1,不需要

	/* Note: here we are figuring rscale in base-NBASE digits */
	rscale1 = var1->ndigits - var1->weight - 1;  // -1是因为权值跟实际的"base-10000"的进制位数相差1
	rscale2 = var2->ndigits - var2->weight - 1;
	res_rscale = Max(rscale1, rscale2);

	res_ndigits = res_rscale + res_weight + 1;  // 同上,权值问题
	if (res_ndigits <= 0)  // 最少一位
		res_ndigits = 1;

	res_buf = digitbuf_alloc(res_ndigits + 1);  // 预留一个"NumericDigit用于可能的进位/rouding"
	...
	for (i = res_ndigits - 1; i >= 0; i--)
	{
		i1--;
		i2--;
		if (i1 >= 0 && i1 < var1ndigits)  // int16范围:-32768~+32767,肯定不会加越界
			carry += var1digits[i1];
...
round_var(NumericVar *var, int rscale)
{
	...
	di = (var->weight + 1) * DEC_DIGITS + rscale;  // 这里的rscale就是十进制位数,区别于var->rscale(int16为单位)

	/*
	 * If di = 0, the value loses all digits, but could round up to 1 if its
	 * first extra digit is >= 5.  If di < 0 the result must be 0.
	 */
	if (di < 0)  // 说明进位跑出最高权值的位置
	{
		var->ndigits = 0;
		var->weight = 0;
		var->sign = NUMERIC_POS;
	...
	{  // 最外层else start
		/* NBASE digits wanted */  // 计算进位相当于多少个int16
        // 这一步已经包含了di=0,ndigits计算的结果为0
		ndigits = (di + DEC_DIGITS - 1) / DEC_DIGITS;

		/* 0, or number of decimal digits to keep in last NBASE digit */
		di %= DEC_DIGITS;  // 最后一个int16保存的实际位数
...
strip_var(NumericVar *var)
{
	...
	/* If it's zero, normalize the sign and weight */  // 标准化
	if (ndigits == 0)
	{
		var->sign = NUMERIC_POS;
		var->weight = 0;
