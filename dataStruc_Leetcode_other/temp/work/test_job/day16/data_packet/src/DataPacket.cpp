...
void DataPacket::RandInit(int upper_limit)
{
	...
	for (int i = 0; i < upper_limit; i++)
	{
		...
		// random int value 0 ~ 65535	// for test
		//(this->i_vec).push_back( rand() % 65536 );
		//cout << this->i_vec[i] << endl; 
		//usleep(10000);

		// random int value 0 ~ 65535
		(this->i_vec).push_back( rand() % 65536 );
		// random long value 0 ~ 65535
		(this->l_vec).push_back( rand() % (long)65536 );
		// random double value 0.0 ~ 65535.0
		(this->d_vec).push_back( rand() / (double)(RAND_MAX / 65536) );
		(this->str_vec).push_back( GetRandomStr() );
	}
